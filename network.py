...
class Network:
	...
	def build_network(self, inputs, labels, is_training, train):
		...
		if (self.config == '--MNIST'):
			...
			kernel_shape = [1, 1, 128, self.channels]
			bias_shape = [self.channels]
		...
		if (train == 'train'):
			...
			if (self.config == '--MNIST'):
				...
				self.NLL = tf.reduce_sum(tf.nn.sparse_cross_entropy_with_logits(labels=tf.cast(labels, tf.int8), logits=network))
		...
		with tf.Session() as sess:
			...
			for i in range(self.test_inputs.shape[0]):
				if (self.config == '--MNIST'):
					...
					image = np.reshape(image, (1, 28, 20, 1))
					loss.append(sess.run(self.loss, feed_dict={x:image, y:image, is_training: training}))
					NLL.append(sess.run(self.NLL, feed_dict={x:image, y:image, is_training: training}))
				...
				for j in range(self.width):
					for k in range(self.channels):
						...
						if (self.config == '--MNIST'):
							...
							sample = sample_categorical(probs[:, i, j, k])
